...
function createFlagBearers({ world }) {
  ...
  const handGeometry = new THREE.SphereGeometry(0.075, 10, 6);
  const bodyGeometry = new THREE.CylinderGeometry(0.14, 0.18, 0.46, 10);
  const limbGeometry = new THREE.CylinderGeometry(0.045, 0.055, 1, 8);
  const up = new THREE.Vector3(0, 1, 0);
  const figures = [];

  const createLimb = (group, from, to) => {
  ...
  [-1, 1].forEach((side, index) => {
    ...
    group.renderOrder = 5;
    world.add(group);
    figures.push({ group, legs, side, phase: index * Math.PI });
  });
  ...
  return {
    update(morphProgress, time, scrollProgress = 0) {
      ...
      figures.forEach(({ group, legs, side, phase }) => {
        const motionPhase = scrollProgress * Math.PI * 22 + time * 2 + phase;
        const stride = Math.sin(motionPhase);
        const bounce = Math.abs(Math.sin(motionPhase));
        group.position.y = -0.64 + bounce * 0.1 * visibility;
        group.rotation.z = stride * 0.07 * visibility;
    ...
    dispose() {
      figures.forEach(({ group }) => world.remove(group));
...
export async function createBlobMeshes({ world }) {
  ...
  const flagBearers = createFlagBearers({ world });
