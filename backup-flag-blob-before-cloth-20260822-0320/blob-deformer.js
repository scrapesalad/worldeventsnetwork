...
export function createBlobDeformer({
  ...
  return function deformBlob(geometry, time, options = {}) {
    ...
    for (let i = 0; i < vertexCount; i += 1) {
      ...
      // Keep the pre-explosion form broad and flag-like without changing the cube stage.
      const rectangularMix = 1 - smoothstep(0.34, 0.7, morphProgress);
      px *= THREE.MathUtils.lerp(1, 1.48, rectangularMix);
      py *= THREE.MathUtils.lerp(1, 0.82, rectangularMix);
      pz *= THREE.MathUtils.lerp(1, 0.68, rectangularMix);
