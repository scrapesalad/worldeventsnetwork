...
async function createCrowdSilhouettes({ world }) {
  ...
  return {
    update(morphProgress, time) {
      const visibility = 1 - smoothstep(0.24, 0.7, morphProgress);
      material.opacity = visibility * 0.82;
      sprites.forEach((sprite, index) => {
        const phase = index * 0.62;
        sprite.position.y = -1.12 + Math.sin(time * 2.1 + phase) * 0.13 * visibility;
        sprite.position.z = 1.7 + Math.cos(time * 1.7 + phase) * 0.04;
        sprite.rotation.z = Math.sin(time * 2.1 + phase) * 0.12 * visibility;
      });
    },
    dispose() {
